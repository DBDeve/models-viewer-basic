...
export class viewer {
        ...
        try {
            ...
            if(config.scene.model){
                ...
                */
                ...
                model.noModelMessage(model,config.scene.noModel.Message, config.scene.noModel.defaultPath)
                //model.setDragDrop()
            }
    ...
    noModelMessage(model,message, defaultPath) {
        ...
        //trovare un modo per passare qui il path del modello di default
        async function addModel(model,defaultPath) {
            ...
            model.managementScene.scene.traverse((object) => {
                if (object.isGroup) {
                    object.clear();  
                }}
            ); 
            ...
            if(typeFile==='gltf' | typeFile==="glb"){
                modelFile= await model.managementScene.load.fileGLTF(defaultPath)
            }
            else if(typeFile==="fbx"){
                modelFile = await model.managementScene.load.fileFBX(defaultPath);
                ...
                modelFile = await model.managementScene.load.fileOBJ(defaultPath, modelConfig.texturePath)
                ...
                modelFile = await model.managementScene.load.filePLY(defaultPath)
            }
            else if(typeFile==='stl'){
                modelFile = await model.managementScene.load.fileSTL(defaultPath)
            }
            else if(typeFile==='dae'){
                modelFile = await model.managementScene.load.fileCollada(defaultPath)
                modelFile.traverse((child) => {
                    if (child.isMesh) {
                        console.log(child);
                        child.material= new THREE.MeshStandardMaterial({
            ...
            model.managementScene.scene.add(modelFile);
            model.ManagementRenderer.animations.isDownloadingFirstModel=false;
            ...
            model.ManagementCamera.setCamera(config.camera.basic, distance); //trovare un modo per passare la configurazione della camera
